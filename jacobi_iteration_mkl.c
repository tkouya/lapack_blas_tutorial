...
int jacobi_iteration_coo(double *answer, double *mat_a_val, int *mat_a_row_index, int *mat_a_col_index, int *mat_a_num_nonzeros, double *diagonal, double *vec_b, double reps, double aeps, double history_norm2[], int dim, int maxtimes)
{
	int i, itimes;
	...
	for(itimes = 0; itimes < maxtimes; itimes++)
	{
		...
		// y := b - A * x
		mkl_cspblas_dcoogemv("N", &dim, mat_a_val, mat_a_row_index, mat_a_col_index, mat_a_num_nonzeros, vx, vtmp);
...
*/
	...
	// Jacobi iteration
	itimes = jacobi_iteration_coo(vx, ma_val, ma_row_index, ma_col_index, &ma_num_nonzero, md, vb, reps, aeps, his_norm2_res, dim, maxtimes);
